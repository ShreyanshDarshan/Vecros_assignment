...
def world_to_index(pos, voxel_size):
    ...
    return (
        ...
        ind[1],
        ind[2],
    )  # tuple((np.array(pos) / voxel_size).astype(int).tolist())
...
def get_path(lock, state, bot_size, bounds, voxel_size, obstacles, path_list):
    ...
    # create costmap
    cost_map = np.zeros(
        (np.array(bounds) // voxel_size.value + np.array([1, 1, 1])).astype(int)
    )
    ...
    while True:
        ...
        if (
            len(path_list) > 6
            and np.linalg.norm(np.array(state[:]) - np.array(destination)) > 10
        ):
            path_formatted = np.reshape(
                np.array(path_list), (int(len(path_list) / 3), 3)
            ).tolist()
...
def has_shorter_path(cur, neighbour, nodes):
    if nodes[neighbour][2] != 1:
        ...
        nodes[cur][0] + np.linalg.norm(np.array(neighbour) - np.array(cur))
        < nodes[neighbour][0]
    ):
        return True
    else:
...
def astar(start_node, target_node, cost_map):
    nodes = np.zeros(
        (cost_map.shape[0], cost_map.shape[1], cost_map.shape[2], 3), dtype=float
    )
    parents = np.ones(
        (cost_map.shape[0], cost_map.shape[1], cost_map.shape[2], 3), dtype=int
    )
    parents = parents * (-1)
    open_nodes = []
    closed_nodes = []
    ...
    while True:
        ...
        for neighbour in get_neighbours(current_node):
            if (
                in_bounds(neighbour, target_node)
                and cost_map[neighbour] == 0
                and nodes[neighbour][2] != 2
            ):
                if (
                    has_shorter_path(current_node, neighbour, nodes)
                    or nodes[neighbour][2] != 1
                ):
